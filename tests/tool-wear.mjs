...
test('durability adapter uses literal current cache-busted imports and stays pure', () => {
  assert.match(durabilitySource, /from ['"]\.\/items\.js\?v=261['"]/);
  assert.match(durabilitySource, /from ['"]\.\/tool-tiers\.js\?v=224['"]/);
  assert.match(durabilitySource, /from ['"]\.\/inventory\.js\?v=224['"]/);
...
test('game mining paths reach durability wear and use the current adapter cache', () => {
  assert.match(gameSource, /from ['"]\.\/durability\.js\?v=227['"]/);
