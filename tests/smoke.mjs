...
test('bug sprint: all visible version surfaces agree', () => {
  ...
  assert.ok(html.includes('v1.18.15'), 'HTML must expose v1.18.15');
...
test('procedural item icons reach hotbars, inventory, and chest without dropping labels', () => {
  ...
  const pub = fsText('public/index.html');
  assert.equal(html, pub, 'root/public HTML must stay identical');
  assert.ok(game.includes("from './item-icons.js?v=15'"));
  ...
  assert.match(html, /\.hotbar-slot:focus-visible[\s\S]*?outline: 2px solid rgba\(143,232,255,0\.92\)/);
